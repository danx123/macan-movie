...
class ModernVideoPlayer(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Menambahkan label judul di tengah
    ...
    def _load_video_file(self, file_path_or_url):
        # /*** MODIFIKASI PERBAIKAN BUG ***/
        self.window().setWindowTitle(f"Macan Player - Memuat...")
        self._stop_video()
        
        # Logika baru untuk menentukan judul yang benar
        new_title = os.path.basename(file_path_or_url) # Default fallback
        if self.current_media_info.get('path') == file_path_or_url:
             new_title = self.current_media_info.get('title', new_title)

        # Sumber judul paling akurat adalah dari data playlist itu sendiri
    ...
    def _stop_video(self):
        ...
        self.subtitle_text_item.setHtml("")
        self.current_subtitle_text = ""
        # Reset label judul dan judul jendela
    ...
    # =================================================================
    # ========== PERBAIKAN FUNGSI MOUSE EVENT DI FULLSCREEN ===========
    # =================================================================
    def eventFilter(self, source, event):
        if source is self.video_widget:
            # BLOK KODE BARU UNTUK MENDETEKSI GERAKAN MOUSE
            # Ini akan mencegat event mouse move langsung dari video_widget
            if event.type() == QEvent.Type.MouseMove:
                if self.is_fullscreen:
                    self.controls_container.setVisible(True)
                    self.setCursor(Qt.CursorShape.ArrowCursor)
                    self.controls_hide_timer.start()
                # Kita return False agar event bisa diproses lebih lanjut jika perlu
                return False
