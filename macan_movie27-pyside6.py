...
from PySide6.QtCore import (
    ...
    Signal, QObject, QThread, Slot, QRectF, QTime, QBuffer
)
...
# /** MODIFIKASI **/ Blok impor untuk library 'yt_dlp' telah dihapus.
# Fitur URL sekarang akan menggunakan 'yt-dlp.exe' secara langsung.
    
# /** MODIFIKASI BARU **/ Ditambahkan untuk fitur TV Online
try:
    import requests
except ImportError:
    print("Peringatan: Pustaka 'requests' tidak ditemukan. Fitur TV Online tidak akan berfungsi.")
...
# /*** MODIFIKASI ***/
# Kelas MiniPlayerWindow telah dihapus karena tidak lagi digunakan.
...
# /*** MODIFIKASI BARU ***/
# Menambahkan manajer ikon SVG khusus untuk kontrol pemutar video
class PlayerIconManager:
    SVG_DATA = {
        ...
        "folder-open": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>""",
        ...
        "step-backward": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 19 2 12 11 5 11 19"></polygon><polygon points="22 19 13 12 22 5 22 19"></polygon></svg>""",
        "step-forward": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 4 14 12 5 20 5 4"></polygon><line x1="19" y1="5" x2="19" y2="19"></line></svg>""",
        ...
        "volume-mute": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>""",
        "volume-down": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg>""",
        "volume-up": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg>""",
        "expand": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"></path></svg>""",
        "compress": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 14h6v6m-6-6l7 7m13-11h-6V4m6 6l-7-7M4 10V4h6m-6 0l7 7m13 3v6h-6m6-6l-7 7"></path></svg>""",
        "palette": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0L12 2.69z"></path></svg>""",
    ...
    @staticmethod
    def get_icon(name: str, color: str = "#ecf0f1", size: QSize = QSize(22, 22)) -> QIcon:
        xml_data = PlayerIconManager.SVG_DATA.get(name)
...
# /** MODIFIKASI **/
# Kelas ini diubah untuk menggunakan yt-dlp.exe melalui subprocess,
# bukan mengimpor library yt_dlp secara langsung.
class YouTubeDLWorker(QObject):
    ...
    def run(self):
        """Mencari dan menjalankan yt-dlp.exe untuk mendapatkan URL video."""
        ytdlp_path = "yt-dlp.exe"
        # Jika dijalankan sebagai file .exe yang dibundel (misal oleh PyInstaller),
        # cari executable di dalam direktori _MEIPASS.
        if hasattr(sys, "_MEIPASS"):
            ytdlp_path = os.path.join(sys._MEIPASS, ytdlp_path)

        if not os.path.exists(ytdlp_path):
            self.finished.emit(None, None, f"Executable 'yt-dlp.exe' tidak ditemukan.")
            return

        try:
            # Konfigurasi untuk menyembunyikan jendela command prompt di Windows
            startupinfo = None
            creationflags = 0
            ...
            if result.returncode != 0:
                # Jika yt-dlp mengembalikan error
                error_message = result.stderr.strip() if result.stderr else "Error tidak diketahui."
                self.finished.emit(None, None, f"Error dari yt-dlp.exe: {error_message}")
                return

            # Mem-parsing output yang berhasil
            output_lines = result.stdout.strip().split('\n')
            if len(output_lines) >= 2:
                title = output_lines[0]
                video_url = output_lines[-1] # Ambil baris terakhir untuk URL, lebih aman
                self.finished.emit(video_url, title, None)
            else:
                self.finished.emit(None, None, "Gagal mem-parse output dari yt-dlp.exe. Output:\n" + result.stdout)

        except FileNotFoundError:
             self.finished.emit(None, None, f"Perintah '{ytdlp_path}' tidak ditemukan. Pastikan yt-dlp.exe ada di path yang benar.")
        except Exception as e:
            self.finished.emit(None, None, f"Gagal saat menjalankan proses yt-dlp.exe: {str(e)}")
...
class ModernVideoPlayer(QWidget):
    ...
    # /*** MODIFIKASI ***/
    # Menambahkan sinyal untuk minimize dan close
    ...
    def __init__(self):
        ...
        # /*** MODIFIKASI ***/
        # Menambahkan variabel untuk fungsionalitas geser jendela
    ...
    def _apply_theme(self, theme_name):
        if theme_name in self.themes:
            ...
            # /*** MODIFIKASI ***/
            # Menghapus referensi ke mini_player_widget
    ...
    def _setup_ui(self):
        ...
        # JANGAN pasang event filter di sini
        ...
        self.btn_back.setIcon(PlayerIconManager.get_icon('arrow-left'))
        self.btn_back.setToolTip("Kembali ke Koleksi")

        self.btn_open = QPushButton()
        self.btn_open.setIcon(PlayerIconManager.get_icon('folder-open'))
        self.btn_open.setToolTip("Buka File Video")
        self.url_input = QLineEdit()
        self.url_input.setPlaceholderText("URL video (YouTube, dll)...")
        self.btn_load_url = QPushButton()
        self.btn_load_url.setIcon(PlayerIconManager.get_icon('link'))
        self.btn_load_url.setToolTip("Muat dari URL")
        self.btn_toggle_url_bar = QPushButton()
        self.btn_toggle_url_bar.setIcon(PlayerIconManager.get_icon('globe'))
        self.btn_toggle_url_bar.setToolTip("Tampilkan/Sembunyikan Bar URL")
        self.btn_show_playlist = QPushButton()
        self.btn_show_playlist.setIcon(PlayerIconManager.get_icon('list'))
        self.btn_show_playlist.setToolTip("Playlist")
        self.btn_show_history = QPushButton()
        self.btn_show_history.setIcon(PlayerIconManager.get_icon('history'))
        self.btn_show_history.setToolTip("Riwayat Tontonan")
        self.position_slider = ClickableSlider(Qt.Orientation.Horizontal)
        self.position_slider.setRange(0, 0)
        self.time_label = QLabel("00:00 / 00:00")
        self.btn_prev_playlist = QPushButton()
        self.btn_prev_playlist.setIcon(PlayerIconManager.get_icon('step-backward'))
        self.btn_prev_playlist.setToolTip("Video Sebelumnya")
        self.btn_next_playlist = QPushButton()
        self.btn_next_playlist.setIcon(PlayerIconManager.get_icon('step-forward'))
        self.btn_next_playlist.setToolTip("Video Berikutnya")
        self.btn_play_pause = QPushButton()
        self.btn_play_pause.setIcon(PlayerIconManager.get_icon('play'))
        self.btn_play_pause.setToolTip("Play/Pause (Spasi)")
        self.btn_stop = QPushButton()
        self.btn_stop.setIcon(PlayerIconManager.get_icon('stop'))
        self.btn_stop.setToolTip("Stop")
        self.btn_speed = QPushButton(f"{self.playback_speeds[self.current_speed_index]}x")
        self.btn_speed.setToolTip("Kecepatan Putar")
        ...
        self.btn_fullscreen.setIcon(PlayerIconManager.get_icon('expand'))
        self.btn_fullscreen.setToolTip("Layar Penuh (F11)")
        self.btn_change_theme = QPushButton()
        self.btn_change_theme.setIcon(PlayerIconManager.get_icon('palette'))
        self.btn_change_theme.setToolTip(f"Ganti Tema (Sekarang: {self.theme_names[self.current_theme_index]})")

        self.btn_minimize = QPushButton()
        self.btn_minimize.setIcon(PlayerIconManager.get_icon('minimize'))
        self.btn_minimize.setToolTip("Minimize")
        self.btn_close_player = QPushButton()
        self.btn_close_player.setIcon(PlayerIconManager.get_icon('close'))
        self.btn_close_player.setToolTip("Close")
        
        self.video_title_label = QLabel("Tidak ada media yang diputar")
        self.video_title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.video_title_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.video_title_label.setStyleSheet("font-weight: bold; color: #bdc3c7;")
        ...
        bottom_controls_layout.addWidget(self.video_title_label, 1, Qt.AlignmentFlag.AlignCenter)
        ...
        bottom_controls_layout.addWidget(self.btn_minimize)
        bottom_controls_layout.addWidget(self.btn_close_player)
        ...
        # /*** PERBAIKAN ***/
        # Pindahkan pemasangan event filter ke sini, setelah semua widget dibuat
        self.video_widget.installEventFilter(self)
        self.video_title_label.installEventFilter(self)
    ...
    def _connect_signals(self):
        ...
        self.btn_show_history.clicked.connect(self._show_history_window) # Menghubungkan tombol history
        ...
        # /*** MODIFIKASI ***/
        # Menghubungkan tombol minimize dan close baru
        self.btn_minimize.clicked.connect(self.minimize_requested.emit)
        self.btn_close_player.clicked.connect(self.close_requested.emit)
    ...
    # /*** MODIFIKASI ***/
    # Fungsi _show_mini_player dan _show_main_from_mini telah dihapus
    ...
    # /** MODIFIKASI BARU **/ Ditambahkan untuk memutar stream TV online
    def open_url_stream(self, url, title):
        """Mempersiapkan dan memutar stream dari URL dengan judul tertentu."""
        self._stop_video()
        self.current_media_info = {'path': url, 'title': title}
        self.srt_parser = None  # Tidak ada subtitle untuk stream
    ...
    def _load_video_file(self, file_path_or_url):
        ...
        new_title = os.path.basename(file_path_or_url)
        if self.current_media_info.get('path') == file_path_or_url:
             new_title = self.current_media_info.get('title', new_title)

        playlist_data = self.playlist_widget.get_playlist_data()
        for item in playlist_data:
            if item.get('path') == file_path_or_url:
                new_title = item.get('title', new_title)
                break
        ...
        if not is_url:
            ...
            self.srt_parser = None # Pastikan subtitle dikosongkan untuk URL
        
        source = QUrl(file_path_or_url) if is_url else QUrl.fromLocalFile(file_path_or_url)
        
        self.current_media_info = {'path': file_path_or_url, 'title': new_title}
        self.player.setSource(source)
        self.player.play()
        self.window().setWindowTitle(f"Macan Player - {new_title}")
        self.video_title_label.setText(new_title)
        self._update_control_states()
        if not is_url or "youtube.com" in file_path_or_url or "youtu.be" in file_path_or_url:
            self._add_to_history(file_path_or_url, new_title)
    ...
    def _on_youtube_dl_finished(self, video_url, title, error):
        if error or not video_url:
            ...
            return
        # Panggil metode baru untuk memutar stream URL dengan judul yang benar
        self.open_url_stream(video_url, title)
    ...
    def _stop_video(self):
        ...
        self.subtitle_text_item.setHtml("")
        self.current_subtitle_text = ""
        self.video_title_label.setText("Tidak ada media yang diputar")
        self.window().setWindowTitle("Macan Player")
    ...
    def _update_volume_icon(self):
        ...
        if is_muted or volume == 0: icon_name = 'volume-mute'
        elif 0 < volume <= 50: icon_name = 'volume-down'
        else: icon_name = 'volume-up'
        self.btn_mute.setIcon(PlayerIconManager.get_icon(icon_name))
    ...
    def _update_control_states(self):
        ...
        # Tombol minimize dan close selalu aktif
        # self.btn_minimize.setEnabled(is_media_loaded)
        self._update_playlist_nav_buttons()

    def _update_play_pause_icon(self, state):
        is_playing = state == QMediaPlayer.PlaybackState.PlayingState
        icon_name = 'pause' if is_playing else 'play'
        self.btn_play_pause.setIcon(PlayerIconManager.get_icon(icon_name))
    ...
    def set_fullscreen_state(self, is_fullscreen):
        ...
        icon_name = 'compress' if is_fullscreen else 'expand'
        self.btn_fullscreen.setIcon(PlayerIconManager.get_icon(icon_name))
    ...
    # /*** MODIFIKASI ***/
    # Menggabungkan event filter untuk geser jendela dan interaksi video
    def eventFilter(self, source, event):
        # Blok untuk fungsionalitas geser jendela menggunakan label judul
        if source is self.video_title_label:
            if event.type() == QEvent.Type.MouseButtonPress:
                if event.button() == Qt.MouseButton.LeftButton:
                    self.drag_pos = event.globalPosition().toPoint() - self.window().frameGeometry().topLeft()
                    return True  # Mengonsumsi event
        ...
        # Blok asli untuk interaksi widget video
...
SETTINGS_SORT_KEY = "sortMethod" 

# /** MODIFIKASI BARU **/ Konstanta untuk fitur TV Online
TV_SOURCES = {
    "Github (Default ID)": "https://iptv-org.github.io/iptv/countries/id.m3u",
...
class IconManager:
    SVG_DATA = {
        ...
        "tv": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"></rect><polyline points="17 2 12 7 7 2"></polyline></svg>"""
    }
    @staticmethod
...
# /** MODIFIKASI BARU **/ Kelas Dialog untuk TV Online
class OnlineTVDialog(QDialog):
    stream_selected = Signal(str, str)  # url, title

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("TV Online")
        self.setMinimumSize(500, 600)
        self.channels = []
        self._setup_ui()
        self._connect_signals()
        
        # Muat source pertama secara default
    ...
    def _setup_ui(self):
        ...
        # Top layout for source selection
        top_layout = QHBoxLayout()
        source_label = QLabel("Sumber Playlist:")
        ...
        self.search_bar.setPlaceholderText("Cari channel...")
        ...
        # Status label
        self.status_label = QLabel("Pilih sumber untuk memulai.")
    ...
    def on_source_changed(self):
        ...
        url = TV_SOURCES.get(source_name)
        if url and requests:
            self.fetch_channels(url)
        elif not requests:
            QMessageBox.warning(self, "Pustaka Hilang", "Fitur ini memerlukan pustaka 'requests'.\nSilakan install dengan: pip install requests")

    def fetch_channels(self, url):
        self.channel_list.clear()
        self.channel_list.addItem("Mengambil daftar channel...")
        self.status_label.setText(f"Mengunduh dari {url}...")
        QApplication.processEvents()

        try:
            response = requests.get(url, timeout=15)
            response.raise_for_status()
            self.channels = self._parse_m3u_content(response.text)
            
            if not self.channels:
                raise ValueError("Tidak ada channel valid di playlist.")
            
            self.channels.sort(key=lambda x: x['name'])
            self.populate_list()
            self.status_label.setText(f"{len(self.channels)} channel ditemukan.")
        except Exception as e:
            self.channel_list.clear()
            self.channel_list.addItem("Gagal memuat channel.")
            self.status_label.setText("Gagal mengambil daftar channel.")
            QMessageBox.warning(self, "Error", f"Gagal mengambil daftar TV: {e}")
            ...
            if line.startswith("#EXTINF:"):
                try:
                    ...
                    if i + 1 < len(lines) and lines[i+1].strip().startswith('http'):
                        url_stream = lines[i+1].strip()
                        if name and url_stream:
                            parsed_channels.append({'name': name, 'url': url_stream})
                except IndexError:
                    continue
    ...
    def on_item_double_clicked(self, item):
        ...
        channel_data = next((ch for ch in self.channels if ch['name'] == selected_name), None)
        if channel_data:
            self.stream_selected.emit(channel_data['url'], channel_data['name'])
            self.accept()
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        title_label = QLabel("Macan Movie Pro")
        ...
        # /** MODIFIKASI BARU **/ Tombol untuk TV Online
        online_tv_button = QPushButton("TV Online")
        online_tv_button.setIcon(IconManager.get_icon("tv"))
        online_tv_button.clicked.connect(self.show_online_tv_dialog)

        about_button = QPushButton("Tentang")
        about_button.clicked.connect(self.show_about_dialog)
        
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("Cari video...")
        self.search_input.setFixedWidth(250)
        self.search_input.textChanged.connect(self.sort_and_reflow)
        ...
        self.button_layout.addWidget(online_tv_button) # <-- Ditambahkan di sini
        ...
        # /*** MODIFIKASI ***/
        # Menghubungkan sinyal dari pemutar video ke slot jendela utama
        self.player_page.minimize_requested.connect(self.showMinimized)
        self.player_page.close_requested.connect(self.close)
    ...
    def play_video(self, file_path: str):
        ...
        if search_query:
            active_video_list = [
                path for path in active_video_list
                if search_query in os.path.basename(path).lower()
            ]
    ...
    # /** MODIFIKASI BARU **/ Fungsi untuk dialog dan pemutaran TV Online
    def show_online_tv_dialog(self):
        dialog = OnlineTVDialog(self)
        dialog.stream_selected.connect(self.play_online_stream)
        dialog.exec()
        
    def play_online_stream(self, url, title):
        self.player_page.open_url_stream(url, title)
        self.stacked_widget.setCurrentIndex(1)
        if self.isFullScreen():
            self.title_bar.hide()
    ...
    def show_about_dialog(self):
        ...
        about_text = """Macan Movie is a modern video player developed as part of the Macan Angkasa ecosystem <b>Key Features:</b>
                        ...
                        Modern and intuitive interface based on PySide6. 
    ...
    def sort_and_reflow(self):
        temp_video_files = self.video_files[:]
        temp_grouped_videos = {folder: files[:] for folder, files in self.grouped_videos.items()}

        search_query = self.search_input.text().strip().lower()
        if search_query:
            temp_video_files = [
                path for path in temp_video_files 
                if search_query in os.path.basename(path).lower()
            ]
            for folder, files in temp_grouped_videos.items():
                temp_grouped_videos[folder] = [
                    path for path in files 
                    if search_query in os.path.basename(path).lower()
                ]

        sort_key = self.sort_combo.currentText()
        reverse_sort = "(Z-A)" in sort_key or "(Terlama)" in sort_key

        if "Nama" in sort_key:
            sort_func = lambda path: os.path.basename(path).lower()
        elif "Tanggal" in sort_key:
            sort_func = os.path.getmtime
            reverse_sort = not reverse_sort
        
        try:
            temp_video_files.sort(key=sort_func, reverse=reverse_sort)
            for folder in temp_grouped_videos:
                temp_grouped_videos[folder].sort(key=sort_func, reverse=reverse_sort)
        except FileNotFoundError:
            QMessageBox.warning(self, "Sort Error", "File tidak ditemukan saat pengurutan. Harap segarkan folder.")
            return

        self.filtered_video_files = temp_video_files
        self.filtered_grouped_videos = temp_grouped_videos

        self.video_count_label.setText(f"Menampilkan: {len(self.filtered_video_files)}")
        self.reflow_ui()
    ...
    def reflow_ui(self):
        ...
        if self.current_view == "all":
            ...
            if not self.search_input.text().strip():
                ...
                sorted_folders = sorted(self.filtered_grouped_videos.keys())
                
                folder_index = 0
                for folder_path in sorted_folders:
                    video_list = self.filtered_grouped_videos[folder_path]
                    if video_list:
                        row, col = divmod(folder_index, cols)
            ...
            for i, video_path in enumerate(self.filtered_video_files):
            ...
            videos_in_selected_folder = self.filtered_grouped_videos.get(self.selected_folder_path, [])
