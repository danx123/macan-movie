...
class VideoThumbnailWidget(QFrame):
    ...
    def open_player(self):
        """
        FIX: Metode ini sekarang membuat instance dari ModernVideoPlayer
        dan memutar video yang dipilih.
        """
        if not self.video_path:
            QMessageBox.warning(self, "Tidak Ada Video", "Path video tidak valid.")
            return

        # Membuat instance pemutar video modern
        self.player_window = ModernVideoPlayer()
        
        # Memanggil metode untuk memuat dan memutar file video
        self.player_window.open_file_from_path(self.video_path)
        
        # Menampilkan jendela pemutar video
...
class MainWindow(QMainWindow):
    ...
    def reflow_grid(self, video_files_list):
        self.clear_grid()
        cols = max(1, (self.scroll_area.width() - 20) // 240)
