...
import time
import subprocess
import sqlite3  # /** MODIFIKASI **/ Ditambahkan untuk database cache
from PyQt6.QtWidgets import (
    ...
    QProgressDialog, QComboBox, QStackedWidget # /** MODIFIKASI **/ Ditambahkan untuk integrasi player
)
from PyQt6.QtMultimedia import QMediaPlayer, QAudioOutput
from PyQt6.QtMultimediaWidgets import QVideoWidget
from PyQt6.QtCore import (
    Qt, QUrl, QSettings, QTimer, QSize, QPoint, QFileInfo, QEvent,
    pyqtSignal, QObject, QThread, pyqtSlot, QRectF, QTime, QBuffer  # /** MODIFIKASI **/ QBuffer ditambahkan
)
...
class ModernVideoPlayer(QWidget):
    ...
    # /** MODIFIKASI BARU **/ Sinyal untuk kembali ke koleksi dan request fullscreen
    ...
    def _setup_ui(self):
        ...
        # self.setGeometry(100, 100, 700, 550) # Geometri diatur oleh MainWindow
        # icon_path = "player.ico"
        # if hasattr(sys, "_MEIPASS"): icon_path = os.path.join(sys._MEIPASS, icon_path)
        # if os.path.exists(icon_path): self.setWindowIcon(QIcon(icon_path))
        self.video_widget = QVideoWidget()
        ...
        # /** MODIFIKASI BARU **/ Menambahkan tombol back di awal
    ...
    def _connect_signals(self):
        # /** MODIFIKASI BARU **/ Menghubungkan sinyal tombol back
        ...
        # /** MODIFIKASI BARU **/ Mengganti fungsi fullscreen
    ...
    def _show_mini_player(self):
        ...
        # self.hide() # Main window tidak lagi disembunyikan
        self.window().hide() # Sembunyikan top-level window

    def _show_main_from_mini(self):
        self.player.setVideoOutput(self.video_widget)
        # self.show() # Main window tidak lagi ditampilkan seperti ini
        self.window().show() # Tampilkan kembali top-level window
    ...
    def _toggle_fullscreen(self):
        # /** MODIFIKASI BARU **/ Hanya memancarkan sinyal, MainWindow yang akan menangani
        self.fullscreen_requested.emit()

    # /** MODIFIKASI BARU **/ Metode untuk MainWindow mengatur status fullscreen
    def set_fullscreen_state(self, is_fullscreen):
        ...
        if not is_fullscreen:
            # Pastikan kontrol terlihat dan cursor normal saat keluar dari fullscreen
    ...
    def keyPressEvent(self, event):
        key = event.key()
        # /** MODIFIKASI BARU **/ Event F11/Escape ditangani oleh MainWindow via sinyal
...
# /** MODIFIKASI **/
# Blok berikut ditambahkan untuk menangani caching thumbnail dengan SQLite.
# Ini menggantikan fungsi generate_thumbnail() yang lama.
# -- Database Caching for Thumbnails --
class ThumbnailCache:
    """Manages SQLite caching for video thumbnails to speed up loading."""
    def __init__(self, db_path="macan_thumbnail_cache.db"):
    ...
    def get_thumbnail(self, video_path: str) -> QPixmap | None:
        """Retrieves a thumbnail from the cache if it exists and is up-to-date."""
        if not self.conn or not os.path.exists(video_path):
            return None
        try:
            current_mtime = os.path.getmtime(video_path)
            cursor = self.conn.cursor()
            cursor.execute("SELECT last_modified, thumbnail_data FROM thumbnail_cache WHERE video_path=?", (video_path,))
            row = cursor.fetchone()

            if row:
                cached_mtime, thumb_data = row
                if abs(cached_mtime - current_mtime) < 1e-6: # Compare float timestamps
                    pixmap = QPixmap()
                    pixmap.loadFromData(thumb_data)
                    if not pixmap.isNull():
                        return pixmap
        except sqlite3.Error as e:
            print(f"Error fetching thumbnail from cache for {video_path}: {e}")
        return None

    def set_thumbnail(self, video_path: str, pixmap: QPixmap):
        """Saves a thumbnail to the cache."""
        if not self.conn or not os.path.exists(video_path) or pixmap.isNull():
            return
        try:
            current_mtime = os.path.getmtime(video_path)
            ...
            thumb_data = buffer.data()

            cursor = self.conn.cursor()
            cursor.execute("""
                INSERT OR REPLACE INTO thumbnail_cache (video_path, last_modified, thumbnail_data)
                VALUES (?, ?, ?)
            """, (video_path, current_mtime, sqlite3.Binary(thumb_data)))
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    # /** MODIFIKASI BARU **/ Metode untuk mengelola cache
    def get_cache_db_path(self) -> str:
        """Returns the full path to the database file."""
        return self.db_path

    def clear_cache(self):
        """Closes connection, deletes the db file, and re-initializes."""
        self.close()
        try:
            if os.path.exists(self.db_path):
                os.remove(self.db_path)
                print("Thumbnail cache file deleted.")
            # Re-initialize
            self._connect()
            self._create_table()
        except OSError as e:
...
class VideoThumbnailWidget(QFrame):
    def __init__(self, video_path: str, main_window, parent=None):
        ...
        # self.player_window = None # Tidak diperlukan lagi, player terintegrasi
    ...
    def open_player(self):
        if not self.video_path:
            ...
            return
        # /** MODIFIKASI BARU **/ Memanggil metode di MainWindow untuk memutar video
...
class FolderThumbnailWidget(QFrame):
    ...
    # /** MODIFIKASI BARU **/ Konteks menu untuk folder
    def contextMenuEvent(self, event):
        menu = QMenu(self)
...
class ManageFoldersDialog(QDialog):
    def __init__(self, folders, parent=None):
        ...
        self.setMinimumSize(450, 350) # Menambah tinggi untuk widget baru
        icon_path = "player.ico"
        if hasattr(sys, "_MEIPASS"): icon_path = os.path.join(sys._MEIPASS, icon_path)
        if os.path.exists(icon_path): self.setWindowIcon(QIcon(icon_path))
        ...
        # /** MODIFIKASI BARU **/ Bagian Cache Thumbnail
        ...
        self.update_cache_size_label() # Panggilan pertama
        ...
        # /** MODIFIKASI BARU **/ Layout tombol folder
    ...
    # /** MODIFIKASI BARU **/ Metode-metode baru untuk dialog
    def update_cache_size_label(self):
        ...
        db_path = thumbnail_cache_manager.get_cache_db_path()
        if os.path.exists(db_path):
            size_bytes = os.path.getsize(db_path)
            size_mb = size_bytes / (1024 * 1024)
            self.cache_size_label.setText(f"Ukuran Cache: {size_mb:.2f} MB")
        else:
            self.cache_size_label.setText("Ukuran Cache: 0.00 MB")
    ...
    def reset_thumbnail_cache(self):
        ...
        if confirm == QMessageBox.StandardButton.Yes and thumbnail_cache_manager:
            ...
            if self.parent() and hasattr(self.parent(), 'start_video_scan'):
                QTimer.singleShot(100, self.parent().start_video_scan) # Beri jeda sedikit
...
# /** MODIFIKASI BARU **/ Worker untuk scanning video di thread terpisah
...
# -- [MODIFIKASI FINAL] FRAMELESS MAIN WINDOW --
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        
        # /** MODIFIKASI **/
        # Inisialisasi cache manager saat MainWindow dibuat.
        global thumbnail_cache_manager
        ...
        self.current_view = "all" # "all" atau "folder"
        ...
        # /** MODIFIKASI **/ Gunakan QTimer untuk memulai scanning setelah UI ditampilkan
    ...
    def _setup_ui(self):
        ...
        # /** MODIFIKASI BARU **/ Menggunakan QStackedWidget untuk beralih antara koleksi dan player
        main_layout = QVBoxLayout(self.container)
        main_layout.setContentsMargins(0, 0, 0, 0)
        self.stacked_widget = QStackedWidget()
        main_layout.addWidget(self.stacked_widget)

        # --- Halaman 1: Tampilan Koleksi Video ---
    ...
    def mousePressEvent(self, event: QMouseEvent):
        # Hanya izinkan drag jika tampilan koleksi yang aktif
    ...
    # /** MODIFIKASI BARU **/ Metode baru untuk menangani perpindahan tampilan
    def play_video(self, file_path: str):
        """Beralih ke tampilan player dan memutar video yang dipilih."""
        self.player_page.open_file_from_path(file_path)
        self.stacked_widget.setCurrentIndex(1)
        # Sembunyikan title bar dari window utama saat di mode fullscreen player
        if self.isFullScreen():
            self.title_bar.hide()

    def show_collection_view(self):
        """Beralih kembali ke tampilan koleksi dan menghentikan video."""
        self.player_page._stop_video()
        self.stacked_widget.setCurrentIndex(0)
        self.setWindowTitle("Macan Movie - Koleksi Video")
        # Pastikan title bar terlihat lagi
        self.title_bar.show()

    def toggle_main_window_fullscreen(self):
        """Mengatur mode fullscreen untuk seluruh jendela aplikasi."""
        if self.isFullScreen():
            self.showNormal()
            self.player_page.set_fullscreen_state(False)
            self.title_bar.show() # Tampilkan title bar jika keluar fullscreen dari mode player
        else:
            self.showFullScreen()
            self.player_page.set_fullscreen_state(True)
            if self.stacked_widget.currentIndex() == 1: # Sembunyikan jika di mode player
                 self.title_bar.hide()
    ...
    def show_about_dialog(self):
        """Menampilkan dialog 'About' dengan informasi dari file about.txt."""
    ...
    def show_message(self, message):
        """Menampilkan pesan di tengah grid jika tidak ada video."""
        self.clear_layout(self.folders_grid_layout)
        self.clear_layout(self.videos_grid_layout)
    ...
    def reflow_ui(self):
        """Menata ulang UI berdasarkan state (all vs folder)."""
        self.clear_layout(self.folders_grid_layout)
        self.clear_layout(self.videos_grid_layout)
