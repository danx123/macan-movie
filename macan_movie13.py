...
from PyQt6.QtCore import (
    ...
    pyqtSignal, QObject, QThread, pyqtSlot, QRectF, QTime
)
...
def generate_thumbnail(video_path: str, size: QSize = QSize(220, 124)) -> QPixmap:
    try:
        ...
        # Coba ambil frame dari 10% durasi video untuk variasi
        pos = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) * 0.1) 
        cap.set(cv2.CAP_PROP_POS_FRAMES, pos)
        ret, frame = cap.read()
        cap.release()
        if ret:
            # Konversi frame BGR dari OpenCV ke RGB untuk QImage
            rgb_image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            h, w, ch = rgb_image.shape
            bytes_per_line = ch * w
            q_image = QImage(rgb_image.data, w, h, bytes_per_line, QImage.Format.Format_RGB888)
            pixmap = QPixmap.fromImage(q_image)
            # Gunakan KeepAspectRatioByExpanding untuk mengisi area thumbnail, lalu potong
...
class FolderThumbnailWidget(QFrame):
    def __init__(self, folder_path: str, video_list: list, main_window, parent=None):
        ...
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.setObjectName("thumbnailCard")
        # Menggunakan stylesheet yang sama dengan VideoThumbnailWidget
        self.setStyleSheet("""
            #thumbnailCard {
                background-color: #2c3e50;
                border: 1px solid #34495e;
        ...
        # Container untuk grid thumbnail
        ...
        # Label judul folder dan jumlah item
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent):
        # Placeholder: Aksi saat folder di-double-click bisa dikembangkan di sini
        # Misalnya, memfilter tampilan "All Videos" untuk hanya menampilkan isi folder ini
        print(f"Folder '{self.folder_path}' di-klik.")
        super().mouseDoubleClickEvent(event)
...
# -- FRAMELESS MAIN WINDOW (VERSI MODIFIKASI) --
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Data-stores
        self.video_folders = []
        self.video_files = [] # Daftar flat semua video
        self.grouped_videos = {} # Dictionary {folder_path: [video_paths]}
    ...
    def _setup_ui(self):
        ...
        # Title Bar (Sama seperti sebelumnya)
        ...
        # --- Bagian Konten Utama (Struktur Baru) ---
        content_widget = QWidget()
        layout = QVBoxLayout(content_widget)
        layout.setContentsMargins(15, 10, 15, 15)

        # Tombol-tombol (Sama seperti sebelumnya)
        button_layout = QHBoxLayout()
        ...
        # Kontainer di dalam scroll area dengan layout vertikal
        ...
        # --- Bagian untuk Semua Video ---
        self.all_videos_section_label = QLabel("All Videos")
        self.all_videos_section_label.setObjectName("sectionTitle")
        self.all_videos_grid_container = QWidget()
        self.all_videos_grid_layout = QGridLayout(self.all_videos_grid_container)
        self.all_videos_grid_layout.setSpacing(15)

        # Tambahkan semua bagian ke layout utama
        ...
        self.main_content_layout.addWidget(self.all_videos_section_label)
        self.main_content_layout.addWidget(self.all_videos_grid_container)
    ...
    def clear_layout(self, layout):
        """Helper untuk menghapus semua widget dari sebuah layout."""
        while layout.count():
    ...
    def populate_videos(self):
        """Membaca semua file video dari folder dan mengelompokkannya."""
        self.video_files.clear()
        self.grouped_videos.clear()
        ...
        for folder in self.video_folders:
            try:
                if os.path.exists(folder) and os.path.isdir(folder):
                    ...
                    if videos_in_folder:
                        # Urutkan video di dalam folder berdasarkan nama
                        sorted_videos = sorted(videos_in_folder)
                        self.grouped_videos[folder] = sorted_videos
                        self.video_files.extend(sorted_videos)
                else:
                    print(f"Peringatan: Folder tidak ditemukan atau tidak valid: {folder}")
            except Exception as e:
                print(f"Error reading folder {folder}: {e}")
        
        # Urutkan daftar semua video
        ...
        # Panggil reflow untuk menampilkan UI yang sudah diisi data
        self.reflow_ui()

    def show_message(self, message):
        """Menampilkan pesan di tengah grid utama jika tidak ada video."""
        self.clear_layout(self.folders_grid_layout)
        self.clear_layout(self.all_videos_grid_layout)
        self.folders_section_label.hide()
        self.all_videos_section_label.hide()
        
        msg_label = QLabel(message)
        msg_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        msg_label.setStyleSheet("font-size: 18px; color: #7f8c8d;")
        # Tambahkan pesan ke layout all_videos sebagai placeholder
        self.all_videos_grid_layout.addWidget(msg_label, 0, 0, 1, -1, alignment=Qt.AlignmentFlag.AlignCenter)
        
    def reflow_ui(self):
        """Menata ulang semua widget thumbnail (folder dan video) berdasarkan ukuran window."""
        self.clear_layout(self.folders_grid_layout)
        self.clear_layout(self.all_videos_grid_layout)

        # Tampilkan kembali judul jika sebelumnya disembunyikan
        self.folders_section_label.show()
        self.all_videos_section_label.show()

        # Hitung jumlah kolom yang muat
        cols = max(1, (self.scroll_area.width() - 30) // 240) # 240 = lebar widget + spacing

        # --- 1. Isi Grid Folder ---
        if not self.grouped_videos:
            self.folders_section_label.hide()
        else:
            self.folders_section_label.show()
            # Urutkan folder berdasarkan nama path
        ...
        if not self.video_files:
            ...
            for i, video_path in enumerate(self.video_files):
                ...
                self.all_videos_grid_layout.addWidget(thumbnail, row, col)
                
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Gunakan QTimer untuk menunda reflow sedikit, agar lebih efisien
        QTimer.singleShot(50, self.reflow_ui)
