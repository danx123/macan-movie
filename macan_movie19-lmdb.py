...
import time
import subprocess
# /** MODIFIKASI **/ lmdb, struct, dan shutil ditambahkan untuk cache baru
import struct
import shutil
try:
    import lmdb
except ImportError:
    print("Kesalahan: Pustaka 'lmdb' diperlukan untuk caching thumbnail.")
    print("Silakan install dengan: pip install lmdb")
    sys.exit(1)
...
# /** MODIFIKASI TOTAL **/
# Blok berikut diubah total untuk menggunakan LMDB sebagai cache engine.
# -- Database Caching for Thumbnails using LMDB --
class ThumbnailCache:
    """Manages LMDB caching for video thumbnails to speed up loading."""
    def __init__(self, db_name="macan_thumbnail_cache.lmdb"):
        # Path sekarang adalah sebuah direktori
    ...
    def _connect(self):
        try:
            ...
            # Alokasikan map_size yang cukup besar (e.g., 1 GB) untuk thumbnail
            self.env = lmdb.open(self.db_path, map_size=1 * 1024 * 1024 * 1024)
        except lmdb.Error as e:
            print(f"LMDB connection error: {e}")
            self.env = None

    def get_thumbnail(self, video_path: str) -> QPixmap | None:
        """Retrieves a thumbnail from the cache if it exists and is up-to-date."""
        if not self.env or not os.path.exists(video_path):
            return None
        
        try:
            current_mtime = os.path.getmtime(video_path)
            with self.env.begin() as txn:
                key = video_path.encode('utf-8')
                value = txn.get(key)
                if value:
                    # Unpack: 8 byte pertama adalah float (timestamp), sisanya adalah data gambar
                    cached_mtime = struct.unpack('d', value[:8])[0]
                    ...
                    if abs(cached_mtime - current_mtime) < 1e-6:
                        ...
                        if not pixmap.isNull():
                            return pixmap
        except (lmdb.Error, struct.error) as e:
            print(f"Error fetching thumbnail from LMDB cache for {video_path}: {e}")
        return None

    def set_thumbnail(self, video_path: str, pixmap: QPixmap):
        """Saves a thumbnail to the cache."""
        if not self.env or not os.path.exists(video_path) or pixmap.isNull():
            return
        ...
        try:
            ...
            # Pack: 8 byte float (timestamp) + data gambar
            timestamp_bytes = struct.pack('d', current_mtime)
            value = timestamp_bytes + thumb_data

            with self.env.begin(write=True) as txn:
                key = video_path.encode('utf-8')
                txn.put(key, value)
        except lmdb.Error as e:
            print(f"Error caching thumbnail with LMDB for {video_path}: {e}")
    ...
    def clear_cache(self):
        """Closes connection, deletes the db directory, and re-initializes."""
        self.close()
        try:
...
class ManageFoldersDialog(QDialog):
    ...
    # /** MODIFIKASI **/ Metode disesuaikan untuk cache LMDB
        ...
        if os.path.exists(db_path) and os.path.isdir(db_path):
            try:
                ...
                self.cache_size_label.setText(f"Ukuran Cache: {size_mb:.2f} MB")
                 ...
                 self.cache_size_label.setText("Gagal hitung ukuran cache.")
        else:
