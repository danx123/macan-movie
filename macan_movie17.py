...
from PyQt6.QtWidgets import (
    ...
    QProgressDialog, QComboBox # /** MODIFIKASI **/ Ditambahkan untuk fitur baru
)
...
class VideoThumbnailWidget(QFrame):
    def __init__(self, video_path: str, main_window, parent=None):
        ...
        self.player_window = None # Untuk menyimpan referensi ke jendela pemutar video
    ...
    def open_player(self):
        if not self.video_path:
            ...
            return
        self.player_window = ModernVideoPlayer()
        self.player_window.open_file_from_path(self.video_path)
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        manage_folder_button = QPushButton("Manage Folder")
        ...
        # /** MODIFIKASI BARU **/ Tambahkan widget sort by dan total video
        ...
        self.video_count_label = QLabel("Total Videos: 0")
    ...
    # /** MODIFIKASI BARU **/ Metode untuk memulai pemindaian di thread terpisah
    def start_video_scan(self):
        ...
        if not self.video_folders:
            self.show_message("Video not found.\nClick 'Add Folder' to begin.")
            self.video_count_label.setText("Total Videos: 0")
            return
    ...
    # /** MODIFIKASI BARU **/ Slot untuk menangani selesainya pemindaian
    @pyqtSlot(dict, list)
    def on_scan_finished(self, grouped_videos, video_files):
        ...
        if not self.video_files:
            self.show_message("No video files found in the specified folders.")
            self.video_count_label.setText("Total Videos: 0")
        else:
            self.sort_and_reflow()

    # /** MODIFIKASI BARU **/ Slot untuk memperbarui progress dialog
    @pyqtSlot(int, int, str)
    def on_scan_progress(self, current, total, folder_name):
        self.progress_dialog.setValue(current)
        self.progress_dialog.setLabelText(f"Scanning: {folder_name}...")
        QApplication.processEvents() # Pastikan UI tetap responsif

    # /** MODIFIKASI BARU **/ Metode untuk mengurutkan dan menata ulang UI
    def sort_and_reflow(self):
        ...
        elif "Date Modified" in sort_key:
            try:
                self.video_files.sort(key=os.path.getmtime, reverse=("(Newest)" in sort_key))
                for folder in self.grouped_videos:
                    self.grouped_videos[folder].sort(key=os.path.getmtime, reverse=("(Newest)" in sort_key))
            except FileNotFoundError:
                QMessageBox.warning(self, "Sort Error", "A file was not found during sorting. Please refresh folders.")
                return

        self.video_count_label.setText(f"Total Videos: {len(self.video_files)}")
        self.reflow_ui()
    ...
    def reflow_ui(self):
        ...
        if self.current_view == "all":
            ...
            self.videos_section_label.setText(f"Videos in '{folder_name}'")
            self.videos_section_label.show()
            self.videos_grid_container.show()
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if self.video_files: # Hanya reflow jika ada video
    ...
    def closeEvent(self, event):
        ...
        # /** MODIFIKASI **/
        # Menutup koneksi database dengan aman saat aplikasi keluar.
        if thumbnail_cache_manager:
