...
class ModernVideoPlayer(QWidget):
    ...
    def _setup_ui(self):
        ...
        # /*** MODIFIKASI BARU ***/ Label untuk judul video
        ...
        # /*** MODIFIKASI BARU ***/ Menambahkan label judul di tengah
    ...
    def _load_video_file(self, file_path_or_url):
        ...
        # /*** MODIFIKASI BARU ***/ Update label judul video
    ...
    def _stop_video(self):
        ...
        self.subtitle_text_item.setHtml("")
        self.current_subtitle_text = ""
        # /*** MODIFIKASI BARU ***/ Reset label judul dan judul jendela
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # /*** MODIFIKASI BARU ***/ Widget untuk pencarian
        ...
        self.button_layout.addWidget(self.search_input) # /*** MODIFIKASI BARU ***/
    ...
    def play_video(self, file_path: str):
        ...
        # /*** MODIFIKASI BARU ***/ Filter daftar putar berdasarkan pencarian saat ini
    ...
    def sort_and_reflow(self):
        # *** MODIFIKASI BARU ***/ Fungsi ini sekarang menangani pengurutan dan pemfilteran
        
        # Salin daftar asli untuk diolah
        temp_video_files = self.video_files[:]
        temp_grouped_videos = {folder: files[:] for folder, files in self.grouped_videos.items()}

        # 1. Lakukan pemfilteran berdasarkan pencarian
        search_query = self.search_input.text().strip().lower()
        if search_query:
            # Filter daftar semua video
            temp_video_files = [
                path for path in temp_video_files 
                if search_query in os.path.basename(path).lower()
            ]
            # Filter video di dalam setiap folder
        ...
        # 2. Lakukan pengurutan pada daftar yang sudah difilter
        ...
        if "Nama" in sort_key:
            ...
            reverse_sort = not reverse_sort # "Terbaru" berarti descending
    ...
    def reflow_ui(self):
        # *** MODIFIKASI BARU ***/ Menggunakan daftar yang sudah difilter dan diurutkan
        self.clear_layout(self.folders_grid_layout)
        self.clear_layout(self.videos_grid_layout)
        ...
        if self.current_view == "all":
            ...
            # Tampilkan folder hanya jika tidak ada filter pencarian aktif
            if not self.search_input.text().strip():
                self.folders_section_label.show()
                self.folders_grid_container.show()
                
                # Urutkan path folder berdasarkan nama untuk konsistensi
                sorted_folders = sorted(self.filtered_grouped_videos.keys())
                
                folder_index = 0
                for folder_path in sorted_folders:
                    # Tampilkan folder hanya jika berisi video setelah difilter
            ...
            # Tampilkan semua video yang cocok dengan filter
