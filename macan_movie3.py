...
from PyQt6.QtCore import Qt, QUrl, QSettings, QTimer, QSize, QPoint, QPropertyAnimation, QEasingCurve, QFileInfo
...
# -- Manajer Ikon SVG --
# Ikon SVG didefinisikan sebagai string XML untuk portabilitas
# Ikon didesain oleh Google (Material Icons) dan Feather Icons, dilisensikan di bawah Apache 2.0
class IconManager:
...
def generate_thumbnail(video_path: str, size: QSize = QSize(220, 124)) -> QPixmap:
    ...
    try:
        ...
        pos = int(frame_count * 0.1) # Ambil frame dari 10% durasi
...
class VideoPlayerWindow(QMainWindow):
    ...
    def _setup_ui(self):
        # Widget utama menjadi kontainer
        central_widget = QWidget(self)
        self.setCentralWidget(central_widget)
        
        # Layout utama menggunakan QVBoxLayout agar video memenuhi jendela
        ...
        # Animasi untuk panel kontrol
        self.controls_animation = QPropertyAnimation(self.controls_widget, b"opacity")
        self.controls_animation.setDuration(300)
        self.controls_animation.setEasingCurve(QEasingCurve.Type.InOutQuad)

        # Layout untuk kontrol
        ...
        self.volume_slider.hide() # Sembunyikan slider awal
        ...
        for btn in [self.play_pause_button, self.volume_button, self.fullscreen_button]:
            btn.setFlat(True)
    ...
    def _setup_player(self):
        ...
        # Set ikon menggunakan IconManager
    ...
    def hide_controls(self):
        if self.isFullScreen() and self.is_playing:
            self.controls_animation.setDirection(QPropertyAnimation.Direction.Backward)
            self.controls_animation.start()
            self.setCursor(Qt.CursorShape.BlankCursor)

    def show_controls(self):
        self.controls_widget.setProperty("opacity", 0) # Set state awal
        self.controls_animation.setDirection(QPropertyAnimation.Direction.Forward)
        self.controls_animation.start()
        self.controls_widget.show()
        # FIX: Pastikan kontrol selalu di atas video
    ...
    def resizeEvent(self, event):
        ...
        # Posisi panel kontrol di bawah tengah
        width = self.width() * 0.8
        height = 100
        x = (self.width() - width) / 2
        y = self.height() - height - 20
        self.controls_widget.setGeometry(int(x), int(y), int(width), int(height))
        # FIX: Pastikan kontrol selalu di atas video saat ukuran jendela berubah
        self.controls_widget.raise_()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if key in (Qt.Key.Key_F, Qt.Key.Key_F11):
            ...
            self.media_player.setPosition(self.media_player.position() + 5000) # Maju 5 detik
        elif key == Qt.Key.Key_Left:
            self.media_player.setPosition(self.media_player.position() - 5000) # Mundur 5 detik
...
class VideoThumbnailWidget(QFrame):
    def __init__(self, video_path: str, main_window, parent=None):
        super().__init__(parent)
        self.video_path = video_path
        self.main_window = main_window # Referensi ke jendela utama
    ...
    def contextMenuEvent(self, event):
        # FIX: Menambahkan dan mengatur ulang konteks menu
        menu = QMenu(self)
        ...
        # Opsi "Remove": Hapus dari tampilan saja
        remove_action = QAction("Hapus dari Tampilan", self)
        remove_action.triggered.connect(self.deleteLater) # deleteLater() akan menghapus widget ini dari layout
        menu.addAction(remove_action)
        
        # Opsi "Delete": Hapus file secara permanen
    ...
    def delete_file(self):
        confirm = QMessageBox.warning(
            self,
            "Konfirmasi Hapus Permanen", # Judul lebih jelas
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.main_window.populate_videos() # Refresh galeri
            except OSError as e:
...
class MainWindow(QMainWindow):
    ...
    def populate_videos(self):
        ...
        # FIX: Gunakan os.walk untuk memindai sub-folder secara rekursif
        for folder in self.video_folders:
            try:
                ...
                # Hapus folder yang tidak valid dari daftar dan simpan perubahan
    ...
    def show_message(self, message):
        ...
        self.grid_layout.addWidget(no_video_label, 0, 0, 1, -1) # Span across all columns
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Tunda sedikit reflow agar tidak terlalu sering dipanggil saat resizing
        QTimer.singleShot(100, self.populate_videos)
        
    def closeEvent(self, event):
        # Simpan geometri jendela saat ditutup
...
if __name__ == "__main__":
    ...
    # Palette gelap untuk tampilan yang konsisten
